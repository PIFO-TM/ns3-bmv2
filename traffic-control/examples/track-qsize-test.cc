...
NS_LOG_COMPONENT_DEFINE ("TrackQueueSize");
...
std::vector<int> flowRates = {10}; // Mbps
  ...
  for (int i = 0; i < numApps; i++)
    {
      ...
      sendRate = std::to_string(flowRates[0]) + "Mbps";
      // Install Sources
...
ConfigTracing ()
{
  ...
  //
  // Configure tracing of the NS3 computed instantaneous queue size
  //
  Ptr<OutputStreamWrapper> ns3QsizeStream = asciiTraceHelper.CreateFileStream (pathOut +"/ns3-inst-qsize.plotme");
  qdisc->TraceConnectWithoutContext ("BytesInQueue", MakeBoundCallback (&InstQueueSizeTrace, ns3QsizeStream));
  //
  ...
  //
  // Configure tracing of the P4 computed instantaneous queue size
  //
  Ptr<OutputStreamWrapper> p4QsizeStream = asciiTraceHelper.CreateFileStream (pathOut +"/p4-inst-qsize.plotme");
  qdisc->TraceConnectWithoutContext ("P4Var1", MakeBoundCallback (&InstQueueSizeTrace, p4QsizeStream));
...
{
  LogComponentEnable ("TrackQueueSize", LOG_LEVEL_INFO);
