...
NS_LOG_COMPONENT_DEFINE ("MicroburstTest");
...
std::vector<int> flowRates = {2, 15}; // Mbps
...
int numApps = 2;
bool enEnqEvents = true;
bool enDeqEvents = true;
...
void
P4VarTrace (Ptr<OutputStreamWrapper> stream, uint32_t oldValue, uint32_t newValue)
{
  *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << newValue << std::endl;
}
  ...
  if (qdiscSelection == "p4")
    {
      ...
      Config::SetDefault ("ns3::P4QueueDisc::EnableEnqueueEvents", BooleanValue (enEnqEvents));
      Config::SetDefault ("ns3::P4QueueDisc::EnableDequeueEvents", BooleanValue (enDeqEvents));
  ...
  for (int i = 0; i < numApps; i++)
    {
      ...
      if (i == 0)
      ...
      else
        {
          ...
          onoff.SetAttribute ("PacketSize", UintegerValue (meanPktSize));
        }
...
ConfigTracing ()
{
  ...
  //
  // Configure tracing of num_culprits
  //
  Ptr<OutputStreamWrapper> numCulpritsStream = asciiTraceHelper.CreateFileStream (pathOut + "/num-culprits.plotme");
  qdisc->TraceConnectWithoutContext ("P4Var1", MakeBoundCallback (&P4VarTrace, numCulpritsStream));
  //
  // Configure tracing of flowID for flow_count measurements
  //
  Ptr<OutputStreamWrapper> flowIdStream = asciiTraceHelper.CreateFileStream (pathOut + "/flow-id.plotme");
  qdisc->TraceConnectWithoutContext ("P4Var2", MakeBoundCallback (&P4VarTrace, flowIdStream));
  //
  // Configure tracing of flow_count measurements
  //
  Ptr<OutputStreamWrapper> flowCountStream = asciiTraceHelper.CreateFileStream (pathOut + "/flow-count.plotme");
  qdisc->TraceConnectWithoutContext ("P4Var3", MakeBoundCallback (&P4VarTrace, flowCountStream));
...
{
  LogComponentEnable ("MicroburstTest", LOG_LEVEL_INFO);
